...
### 5) PasswordPolicy
 ...
 - 사용자가 정한 비밀번호 생성 규칙 저장
 - 비밀번호 제한 사항(길이, 지원하지 않는 문자 등등) 관리
...
|Key| 암호환 및 복호화 하기 위해서 사용되는 데이터이다. Key를 사용해서 암호화하고 복호화 하기 위해서는 같은 Key가 필요하다.|
